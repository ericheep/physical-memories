...
This library can also transform the FFT bins into a Mel Bands, Constant Q bins, or a Chromagram. The implementation of this library will allow for the mechatronic tones to be analyzed in real-time, and that information can then be considered in the creation of the Auditory Distortion Products. Specifically for this project, more research will have to be done to create a robust fundamental frequency function that can accurately determine the base frequency of the mechatronic sound object.
...
By utilizing Music Information Retrieval on the tones created by the mechatronic sound sculptures, a triadic chord can be created whose fundamental is reinforced by the auditory distortion product. Because distortion products are generally masked by sound occupying the same frequency range, the CDT is instead called after the sound of the mechatronic sound object has receded. This allows for a physical sensation to that is related to the sound of mechatronic sound object to exist in the object's wake. The expected CDT can be tuned to the fundamental frequency of the mechatronics, fusing the physical characteristics of the two compositional materials.
